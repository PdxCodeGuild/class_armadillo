...
while True: # loop to print five different emoticons until broken
    question = input('\nWould you like to generate five random emoticons? (yes/no) ').lower().strip() # makes input lower case and removes any extra spaces before and after
    if question == 'yes':
        for x in range(5): # creates one emoticon at a time for five times before looping back up top to ask input again 
            ...
            print('\n' + random_eyes + random_nose + random_mouth) # face manufacture
            time.sleep(1)
