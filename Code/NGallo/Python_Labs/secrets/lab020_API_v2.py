...
def random_quote():
    website_data = requests.get('https://favqs.com/api/qotd')
    json_stuff = json.loads(website_data.text)

    keyword = input("Enter a keyword to find a related quote: ")
    page = input("which page do you want to search: ")
    # website_data = requests.get(f'https://favqs.com/api/quotes?{page}=1&filter={keyword}')
    # json_stuff = json.loads(website_data.text)
    print(json_stuff)
   


json_stuff = random_quote()
