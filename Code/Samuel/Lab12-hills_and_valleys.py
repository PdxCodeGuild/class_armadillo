...
def get_tuple_lakes(data_input):
    ...
    for i in range(len(data_input)):
        ...
        if i != 0 and i != len(data_input):
            # If the value at data_input[i] is equal to the current height, continue.
            if data_input[i] == height:
                ...
                current_peak = -1
                height = -1
            # If the current height is height is lower than the last value in data_input, return list_of_tuples.
            # This is done if the last few values in data_input aren't high enough to meet the current height as to not have floating lakes.
            elif data_input[i] < height and i == len(data_input)-1:
                return list_of_tuples
            # Checks for the first peak it can detect and records the information.
            elif data_input[i] > data_input[i+1] and data_input[i] > data_input[i-1]:
                current_peak = i
                height = data_input[i]
    return list_of_tuples

# Returns a dictionary with lakes filled based on the dictionary given, and the tuples that are given to indicate lakes.
    ...
    for i in range(len(tuples)):
        ...
        for x in range(tuples[i][0],tuples[i][1]):
            ...
            for y in range(tuples[i][2]):
                # If the current value at the given index is not occupied by land ("X"), replace it with water ("0").
                if land[x][y] != "X":
                    land[x][y] = "0"
    return land
...
def print_land(land_input, data_input):
    ...
    while y > -1:
        ...
        while x < len(data_input):
            # If the value at the given indexs is X, change the color of the foreground to Green.
            if land_input[x][y] == "X":
                print(Fore.GREEN, end = "")
            # If the value at the given indexs is 0, change the color of the foreground to Blue.
            elif land_input[x][y] == "0":
                print(Fore.BLUE, end = "")
            # If the index is not at the end of data_input, end with " " instead of "\n".
    ...
    # Prints a line at the bottom of the land based on the length of data_input.
...
# Makes a new dictionary and prints the lakes based on the tuples given from test.
