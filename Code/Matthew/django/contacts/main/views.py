...
def login_page(request):
    if request.method == 'POST':
        ...
        if user is None: # username and password do not match, go back to the page
            return render(request, 'main/login.html', {'message': 'there is no user with that username and password'})
        # user was found, log them in and redirect to the home page
        login(request, user)
        # if there's a next parameter in the url e.g. localhost:8000/main/login/?next=/main/home/
...
def register_page(request):
    if request.method == 'POST':
        ...
        if password != retype_password:
            return render(request, 'main/register.html', {'message': 'passwords do not match'})
        if User.objects.filter(username=username).exists(): # check if a user with that username already exists
            return render(request, 'main/register.html', {'message': 'a user with that username already exists'})
